            ...
            <div class="row">
                <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                    <ul class="breadcrumb breadcrumb-style">
                        <li>
                            <h4 class="page-title">Sales Order Recievables Report</h4>
                        </li>
                    </ul>
                </div>
            </div>
                ...
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
                         ...
                         <h5 style="color:black;">Sales Order Recievables Reports</h5>
                        </div>
                            <br/>
                            <h5 id="range"></h5>
                            ...
                            <div class="table-responsive">
                                <table>
                                        ...
                                        <tr class="top-line nb">
                                                <td colspan="4" class="text-center"><b>Total Recievables</b></td>
                                                <td id="tamount">
                                        </tr>
...
<script>
        window.onload = function () {
            ...
            $('.nav-so-sorecievables').addClass('active');
...
$('#view-content').on('click', function(){
    ...
    $('#range').empty();
    $('#range').append('Sales Order Recievables Summary  '+from+' - ' +to);
    $('.hide-this').show();
   ...
   const salesorder = (from,to) => {
       fetch('/recievable/so-order/'+from+'/'+to)
        .then((res) => res.json())
            .then((data) =>{
                ...
                data.forEach(objso => {
                        ...
                        if (objso.trip_id == null) {
                            ...
                            // '<td>'+(objso.business_name == null) ? objso.name : objso.business_name+'</td>'+
                            '<td>'+objso.type+'</td>'+
                            '<td><b>'+moneyFormat(objso.balance)+'</b></td>'+
                            '</tr>');
                             totalremits += Number(objso.balance);
                        } else {
                            $('#orders').append('<tr class="nb">'+
                            '<td>DR-'+padDigits(objso.id,8)+'</td>'+
